...
public class NotificationHandler {
	...
	void handlePosted(StatusBarNotification sbn) {
		...
		log(DatabaseHelper.PostedEntry.TABLE_NAME, DatabaseHelper.PostedEntry.COLUMN_NAME_CONTENT, no.toString(),no.appName);
	}
	...
	void handleRemoved(StatusBarNotification sbn, int reason) {
		...
		log(DatabaseHelper.RemovedEntry.TABLE_NAME, DatabaseHelper.RemovedEntry.COLUMN_NAME_CONTENT, no.toString(),no.appName);
	}

	private void log(String tableName, String columnName, String content,String app) {
		try {
			if(content != null) {
				synchronized (LOCK) {
					DatabaseHelper dbHelper = new DatabaseHelper(context);
					db_handler dh = new db_handler(context);
					if(app!=null) {
						dh.if_exist(app);
					}
